//
//  YouTubeVideoViewController2.swift
//  PupillometryApp
//
//  Created by Claude Code on 25/07/25.
//  YouTube Ad Video 2 - Enhanced content type tracking for dashboard analytics
//

import UIKit
import WebKit

class YouTubeVideoViewController2: UIViewController {
    ...
    // YouTube video configuration - Video 2
    private let videoID = "7oBZ8sBjdyQ"  // Pepsi advertisement
    private let videoTitle = "Pepsi - For the Love of It"
    private let contentType: ContentType = .youtubeVideo2
    
    // Timing and progress tracking
    private var videoStartTime: TimeInterval = 0
    private var currentVideoTime: TimeInterval = 0
    private var videoDuration: TimeInterval = 45.0  // Longer educational ad
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if !pupillometryManager.isCurrentlyRecording {
            print("🎞️ YouTubeVideo2: Resuming pupillometry recording")
            pupillometryManager.startSession()
        } else {
            print("🎞️ YouTubeVideo2: Pupillometry recording already active")
        }
    ...
    private func createProgrammaticUI() {
        // Create instruction label with different messaging for Video 2
        ...
        instructionLbl.text = "📚 Now watch this Pepsi advertisement for comparison.\\n\\nWe'll analyze your attention patterns to both cola brands."
        ...
        progressVw.progressTintColor = .systemGreen  // Different color for Video 2
        ...
        continueBtn.setTitle("Continue to Dr Pepper", for: .normal)
    ...
    private func configureStoryboardUI() {
        instructionLabel?.text = "📚 Now watch this Pepsi advertisement for comparison.\\n\\nWe'll analyze your attention patterns to both cola brands."
        continueButton?.setTitle("Continue to Dr Pepper", for: .normal)
    ...
    private func configureVideoTracking() {
        print("🎬 YouTubeVideoViewController2: Configuring educational video tracking for \\(contentType.displayName)")
    }
    ...
    private func startVideoTracking() {
        ...
        print("▶️ Educational video tracking started: \\(videoTitle)")
    }
    ...
    private func stopVideoTracking() {
        ...
        print("⏹️ Educational video tracking stopped")
    }
    ...
    private func logVideoEvent(_ eventType: TaskEvent.EventType, data: [String: Any]) {
        ...
        print("📊 Educational video event logged: \\(eventType.rawValue) - \\(contentType.rawValue)")
    }
    ...
    @objc private func continueButtonTapped() {
        stopVideoTracking()
        print("➡️ YouTubeVideo2: Navigating to Dr Pepper video")
        navigateToNextVideo()
    }
    
    private func navigateToNextVideo() {
        let youtubeVC3 = YouTubeVideoViewController3()
        youtubeVC3.title = "Dr Pepper Advertisement"
        navigationController?.pushViewController(youtubeVC3, animated: true)
    }
    
    private func completeConsumerSession() {
        print("🏁 YouTubeVideo2: Completing consumer pathway session")
    ...
    @objc private func skipButtonTapped() {
        ...
        print("⏭️ YouTubeVideo2: Skipping to Dr Pepper video")
        navigateToNextVideo()
    }
}
...
extension YouTubeVideoViewController2: WKNavigationDelegate {
    
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        print("📺 Educational YouTube video loaded successfully")
        startVideoTracking()
    }
    
    func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
        print("❌ Educational YouTube video failed to load: \\(error.localizedDescription)")
        
        DispatchQueue.main.async { [weak self] in
            let alert = UIAlertController(
                title: "Educational Video Load Error",
                message: "Unable to load the educational video. Please check your internet connection.",
